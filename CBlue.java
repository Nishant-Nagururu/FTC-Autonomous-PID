...
@Autonomous(name="Chandrayan Blue", group="Autonomous")
public class CBlue extends LinearOpMode
{
    DcMotor leftMotor, rightMotor, leftBack, rightBack, up, linear;
    BNO055IMU imu;
    ...
    int distToRightTwo = distToRightOne - 30;
    int distMiddle = 11;
    int distRight = 30;
...
//
    ...
    private void leftBlock(){
        ...
        side(-3.15 + leftMove, distToRightOne);
        ...
        side(4.1 - leftMove, distRight);//power 0.7
    ...
    private void rightBlock(){
        ...
        linear.setTargetPosition(1700);
        linear.setPower(1);
        linear.setMode(DcMotor.RunMode.RUN_TO_POSITION);

        side(4.1 + rightMove, 0);//power 0.7
        ...
        side(-4.6 - rightMove + 0.1, distToRightTwo);//power 0.7
...
//        lastBack(1.7, -1);
        ...
        diagLeft(0.2, -1);
        up.setTargetPosition(600);
        driveBack(1.7, 1);
        //driveNormal(0.5, -0.4);
        foundationUnGrab();
        sleep(100);
        driveRightNormal(1.6, 1);
        drive(0.7, 1);
        up.setTargetPosition(0);
        grabber.setPosition(startPos);
        driveRightNormal(0.7, 1);
    }
    ...
    private void driveRelease(double distance, double power, boolean grab){
        ...
        while(opModeIsActive() && Math.abs((leftMotor.getCurrentPosition() + rightMotor.getCurrentPosition())/2)<dist){
            double distanceDriven = Math.abs((leftMotor.getCurrentPosition() + rightMotor.getCurrentPosition())/2);
            ...
            if(distanceRemaining<=600) {
                ...
                leftMotor.setPower(distanceRemaining/divideVal + addVal - correction/2);
                rightMotor.setPower(distanceRemaining/divideVal + addVal + correction/2);
                ...
                correction = pidDrive.performPID(getAngle());
                leftMotor.setPower(distanceDriven/divideVal + addVal - correction/2);
                rightMotor.setPower(distanceDriven/divideVal + addVal + correction/2);
    ...
    private void driveGrab() {
        ...
        while(opModeIsActive() && Math.abs((leftMotor.getCurrentPosition() + rightMotor.getCurrentPosition())/2)<dist) {
            int distanceDriven = (Math.abs(leftMotor.getCurrentPosition()) + Math.abs(rightMotor.getCurrentPosition()))/2;
    ...
    private void driveRightNormal(double distance, double power){
        ...
        while(opModeIsActive() && (Math.abs(leftMotor.getCurrentPosition()) + Math.abs(rightMotor.getCurrentPosition()))/2 < dist){
            int distanceDriven = (Math.abs(leftMotor.getCurrentPosition()) + Math.abs(rightMotor.getCurrentPosition()))/2;
            int distRemaining = dist - distanceDriven;
    ...
    private void side(double distance, int distToWall) {
        ...
        while(opModeIsActive() && (Math.abs(leftMotor.getCurrentPosition()) + Math.abs(rightBack.getCurrentPosition()))/2 < dist){
            int distanceDriven = (Math.abs(leftMotor.getCurrentPosition()) + Math.abs(rightBack.getCurrentPosition()))/2;
            int distRemaining = dist - distanceDriven;
            if(distRemaining <= 500) {
                ...
                rightMotor.setPower((-distRemaining/divideVal - addValue) * 1.08 + correction);
                ...
                rightMotor.setPower((-distanceDriven/divideVal - addValue) * 1.08 + correction);
                leftBack.setPower((-distanceDriven/divideVal - addValue) * 1.2 - correction);
                idle();
            }
            else{
                correction = pidStrafe.performPID(getAngle());
                leftMotor.setPower(leftMotorPower - (correction));
                rightBack.setPower(rightBackPower + (correction));
                rightMotor.setPower(rightMotorPower + (correction));
...
//            else{
            correction = pidStrafe.performPID(getAngle());
            leftMotor.setPower(power - (correction * 1.3));
            rightBack.setPower(power + (correction * 1.3));
            rightMotor.setPower(-power + (correction * 1.3));
    ...
    private void diagRight(double distance, double power) {
        ...
        while(opModeIsActive() && (Math.abs(leftBack.getCurrentPosition()) + Math.abs(rightMotor.getCurrentPosition()))/2 < dist){
            correction = pidDrive.performPID(getAngle());
            leftBack.setPower(power - correction);
            rightMotor.setPower(power + correction);
